...
CommunicationManager::CommunicationManager(PlayerClient& pc, Motion * i, string id, string type)
  :mPlayerClient(pc), mot(i), mNameID(id), mTypeID(type), mIOService(), mSocket(mIOService) 
{
...
void CommunicationManager::do_state_action_campose(){
  ...
  mot->setCurrentPosition(p, time(NULL));
...
void CommunicationManager::do_state_action_pose()
{
  ...
  // Not anymore it gets it directly from Motion
  double xp = 0, yp = 0, ap = 0, confidence = 1.0;
  try {
    Position p = mot->getPosition();  
    xp = p.getX(); 
    yp = p.getY(); 
    ap = p.getTheta();
