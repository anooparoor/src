...
unsigned int auction_t::bids_size()
{
...
std::vector<string> auction_t::get_winners()
{
    ...
    for(int i = 0; i < (int)this->winners.size(); i++)
    {
        ...
        for(bundle_t::iterator j=bundle_won->begin(); j != bundle_won->end(); j++) {
            ...
            sstream ss;
            ss << "SINGLE " << winners[i].first << " AUCTION_WON "
	       << this->id << " " << winners[i].second << " " << j->first << " " << j->second;
...
{
  // E.g.,
  // BROADCAST_TYPE stage AUCTION_START 0 2 3 1 55 55 1 82 73 1 91 147
