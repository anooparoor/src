...
Position MCLPositionEstimator::getPosition(vector<Particle> particles, double& confidence) {
  return getBestCluster(particles, confidence);
  //return getBestK(particles, confidence, 10);
}
...
//of those particles.
Position MCLPositionEstimator::getBestCluster(vector<Particle> particles, double& confidence) {
...
Position MCLPositionEstimator::getBestK(vector<Particle> particles, double& confidence, int n) {
  vector<Particle> particlesToAverage ;

  //  cout << "N: " << n << endl; 
  for ( int i = 0 ; i < n ; i++ ){
    double maxConfidence = 0 ;
    vector<Particle>::iterator iter, pMax;
    for ( iter = particles.begin() ; iter != particles.end(); iter++ ) {
      if ( iter->probability > maxConfidence ) {
	pMax = iter ; 
	maxConfidence = iter->probability; 
      }
    }
    particlesToAverage.push_back(*pMax);
    /*cout << "found max particle: " << *pMax->getPosition().toString()
	 << "\tprobability:" << pMax->probability 
	 << "\txy_probability:" << pMax->xy_probability 
	 << "\ttheta_probability:" << pMax->theta_probability << endl;*/
    particles.erase(pMax);
  }
  //  cout << endl;
