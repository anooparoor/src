 ...
 public:
  ...
  void learnRegions(){
	...
	if(current_circle.inCircle(next_position.getX(), next_position.getY()) && j != k){
	  ...
	  double dist = get_euclidian_distance(current_circle.getCenter().get_x() , current_circle.getCenter().get_y() , x , y);
	...
	double dist = get_euclidian_distance(circles[robotRegion].getCenter().get_x(),circles[robotRegion].getCenter().get_y() ,x, y);
